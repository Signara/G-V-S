   ...
   <body class="landing-page sidebar-collapse">
     ...
     <div class="page-header header-filter" data-parallax="true" style="background-image: url('{{ asset('storage').'/'.$exhibitions->Banner }}')">
       <div class="container">
        <?php
        $curdate = date('Y-m-d');

        $exhbtnid = DB::table('exhibitions')->where('id','=',$exhibitions->id)->where('StartDate','<=',$curdate)->where('EndDate','>=',$curdate)->select('id')->first();

        if(!empty($exhbtnid->id)) {
        ?>
         <div class="text-center MAY">
            <?php
            ...
            {
            ?>
            <a href="<?php echo $exhibitions->android_link ?>" target="_blank" class="btn btn-danger btn-raised btn-lg">
                Go To Expo
            </a>
            <?php } else if($devicename == 'iPhone') { ?>
            <a href="<?php echo $exhibitions->ios_link ?>" target="_blank" class="btn btn-danger btn-raised btn-lg">
                Go To Expo
            </a>
            <?php } else { ?>
            <a href="<?php request()->getHttpHost()?>/expo?exhibitionid=<?php echo $exhibitions->id ?>" target="_blank" class="btn btn-danger btn-raised btn-lg">
                Go To Expo
            </a>
            <?php } ?>
            <!---<div class="col-md-6">
         ...
         <?php } else { ?>
            <div class="text-center MAY">
                ...
                {
                ?>
                <a href="<?php echo $exhibitions->android_link ?>" disabled="disabled" target="_blank" class="btn btn-danger btn-raised btn-lg btn-disabled">
                    Go To Expo
                </a>
                <?php } else if($devicename == 'iPhone') { ?>
                <a href="<?php echo $exhibitions->ios_link ?>" disabled="disabled" target="_blank" class="btn btn-danger btn-raised btn-lg btn-disabled">
                    Go To Expo
                </a>
                <?php } else { ?>
                <a href="<?php request()->getHttpHost()?>/expo?exhibitionid=<?php echo $exhibitions->id ?>" disabled="disabled" target="_blank" class="btn btn-danger btn-raised btn-lg btn-disabled">
                    Go To Expo
                </a>
                <?php } ?>
                <!---<div class="col-md-6">
         ...
         <div class="text-center section">
           <div class="row">
             <div class="ml-auto mr-auto col-md-8">
               <h2 class="title">{{ $exhibitions->Name }}</h2>
               <h5 class="description">{{ Str::limit($exhibitions->Description, '700') }}</h5>
               <h6>{{ $exhibitions->keywords }}</h6>
             </div>
           </div>
         </div>
         <div class="text-center section">
            <h2 class="title">About Organiser</h2>
            <div class="team">
              <div class="row">
                    ...
                    <div class="row">
                      <div class="col-md-5">
                        <div class="card-header card-header-image">
                            @if ($organiser->Logo == '')
                            <img  alt="Profile Image" class="img" src="{{ asset('material') }}/img/bg10.jpg">
                            @else
                            <img class="img" src="{{ asset('storage').'/'.$organiser->Logo }}">
                            @endif
                        </div>
                      </div>
                      <div class="col-md-7">
                        <div class="card-body">
                            <h4 class="card-title">{{ $organiser->CommonName }}</h4>
                          <?php
                             $sectorList = [];
                             $sectordata = DB::table('sectors')->select('name')->whereIn('id', explode(',',$organiser->Sectors))->get()->toArray();
                          ...
                          <h5 class="category text-muted">{{ $sectorName }}</h5>
                          <?php
                             $categoryList = [];
                             $categorydata = DB::table('categories')->select('name')->whereIn('id', explode(',',$organiser->Categories))->get()->toArray();
                          ...
                          <h6 class="category text-muted">{{ $categoryName }}</h6>
                          <p class="card-description">
                            {{ Str::limit($organiser->Description, '50') }}
                          </p>
                        </div>
          ...
          <div class="text-center section">
            ...
            <div class="team">
              <div class="row">
                ...
                <?php $companies = Company::where('id','=',$sponsor->Company)->first(); ?>
                 <div class="col-md-6">
                  <div class="card card-profile card-plain">
                    <div class="row">
                      <div class="col-md-5">
                        <div class="card-header card-header-image">
                          <a href="">
                            ...
                            <img  alt="Profile Image" class="img" src="{{ asset('material') }}/img/bg10.jpg">
                            @else
                            <img class="img" src="{{ asset('storage').'/'.$companies->Logo }}">
                            @endif
                          </a>
                        </div>
                      </div>
                      <div class="col-md-7">
                        <div class="card-body">
                            <h4 class="card-title">{{ $companies->CommonName }}</h4>
                          <?php
                             $sectorList = [];
                          ...
                          <h6 class="category text-muted">{{ $categoryName }}</h6>
                          <p class="card-description">
                            {{ Str::limit($companies->Description, '50') }}
                          </p>
                        </div>
                        <div class="card-footer justify-content-center">
                          <a href="#pablo" class="btn btn-just-icon btn-link btn-twitter">
                ...
                <div class="col-md-6">
                 <div class="card card-profile card-plain">
                   <div class="row">
                     <div class="col-md-5">
                       <div class="card-header card-header-image">
                         <a href="#pablo">
                            ...
                            <img  alt="Profile Image" class="img" src="{{ asset('material') }}/img/bg10.jpg">
                            @else
                            <img class="img" src="{{ asset('storage').'/'.$participant->Logo }}">
                            @endif
                         </a>
                       </div>
                     </div>
                     <div class="col-md-7">
                       <div class="card-body">
                         <h4 class="card-title">{{ $participant->CommonName }}</h4>
                         <?php
                            $sectorList = [];
                            $sectordata = DB::table('sectors')->select('name')->whereIn('id', explode(',',$participant->Sectors))->get()->toArray();
                            foreach ($sectordata as $sector)
                            {
                                $sectorList[] = $sector->name;
                            }
                         ...
                         <?php
                            $categoryList = [];
                            $categorydata = DB::table('categories')->select('name')->whereIn('id', explode(',',$participant->Categories))->get()->toArray();
                            foreach ($categorydata as $category)
                            {
                                $categoryList[] = $category->name;
                            }
                         ...
                         <h6 class="category text-muted">{{ $categoryName }}</h6>
                         <p class="card-description">
                            {{ Str::limit($participant->Description, '50') }}
                         </p>
                       </div>
                       <div class="card-footer justify-content-center">
                         <a href="#pablo" class="btn btn-just-icon btn-link btn-twitter">
         ...
         <div class="about-office">
            ...
            @foreach($exhibitionrelgalleries as $exhibitionrelgallery)
              <div class="col-md-4" style="padding-bottom: 10px;">
                <img class="rounded img-raised img-fluid" alt="Exhibition Image" src="{{ asset('storage').'/'.$exhibitionrelgallery->gallery }}">
              </div>
            @endforeach
            </div>
          </div>
