...
@section('content')
    <div class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <form method="post" enctype="multipart/form-data" action="{{ route('package.store') }}"
                        ...
                        <div class="card ">
                            <div class="card-header card-header-rose card-header-icon">
                                ...
                                <h4 class="card-title">{{ __('Add packages') }}</h4>
                            </div>
                            <div class="card-body ">
                                <div class="row">
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('Cost') ? ' has-danger' : '' }}">
                                            <input class="form-control{{ $errors->has('Cost') ? ' is-invalid' : '' }}"
                                                ...
                                                value="{{ old('Cost') }}" required="true" aria-required="true" />
                                        ...
                                        <div class="form-group{{ $errors->has('ParticipantType') ? ' has-danger' : '' }}">
                                            <select class="pl-0 pr-0 selectpicker col-sm-12" name="ParticipantType"
                                                ...
                                                <option value="{{ $participanttype->id }}"
                                                    {{ in_array($participanttype->id, old('ParticipantType') ?? []) ? 'selected' : '' }}>
                                        ...
                                        <div class="form-group{{ $errors->has('Stalls') ? ' has-danger' : '' }}">
                                            <select class="pl-0 pr-0 selectpicker col-sm-12" name="Stalls[]"
                                                ...
                                                <option value="{{ $model->id }}"
                                                    {{ in_array($model->id, old('Stalls') ?? []) ? 'selected' : '' }}>
                                                    {{ $model->Name }}</option>
                                                @endforeach
                                            </select>
                                ...
                                <div class="row">
                                    <label class="col-sm-2 col-form-label">{{ __('Visitor Data') }}</label>
                                    <div class="col-sm-7">
